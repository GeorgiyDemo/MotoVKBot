...
class MainClass:
        ...
        for event in longpoll.listen():
            ...
            if event.type == VkEventType.MESSAGE_NEW:
                ...
                if event.to_me:
                    ...
                    elif event.text == "Нет" and self.mongo_obj.get_current_step(event.user_id) == 13:
                        self.step_15(event)
                    elif event.text == "Нет" and self.mongo_obj.get_current_step(event.user_id) == 17:
                        self.step_18(event)

                    # Если ответ "Получить купон", то это либо переход к 12, либо к 16
                    elif event.text == "Получить купон" and self.mongo_obj.get_current_step(event.user_id) == 11:
                        self.step_12(event)
                    elif event.text == "Получить купон" and self.mongo_obj.get_current_step(event.user_id) == 15:
                        self.step_16(event)
    ...
    def step_7(self, event):
        ...
        self.vk.method('messages.send', {'user_id': event.user_id, 'random_id': get_random_id(), 'message': message_str,
                                         'attachment': photo_obj.photo_str})  # 'attachment': "market-170171504_3154895"
        time.sleep(2)
        self.step_8(event)
